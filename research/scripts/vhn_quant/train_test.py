...
def train(criterion1, criterion2, optimizer, net, num_epochs, dldr_trn):
    for epoch in range(num_epochs):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(dldr_trn, 0):
            ...
            outputs = net(temp_inputs.type(torch.float32))
            ...
            if criterion2: 
                ...
                for i in range(dldr_trn.batch_size):
                    if int(temp_labels[i]) == 1:
                        ...
                        target_cnt += 1
                target_cnt = 1 if target_cnt == 0 else target_cnt
                x_bar /= target_cnt
                # print(type(torch.tensor(net.vhn.weights)))

                _temp_wghts = np.zeros((dldr_trn.batch_size, net.WIRES, 32, 32, 50))

                for i in range(dldr_trn.batch_size):
                    for k in range(net.WIRES):
                        _temp_wghts[i,k] = net.vhn.weights.detach().numpy()
                
                _temp_wghts = torch.tensor(_temp_wghts).type(torch.float32)
                loss += criterion2(curly_Nprime(_temp_wghts), curly_N(torch.tensor(x_bar).type(torch.float32)))
                loss = loss.float()
